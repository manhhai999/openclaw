...
export function buildToolCardSidebarContent(card: ToolCard): string {
  ...
  if (card.inputText?.trim()) {
    ...
    sections.push(
      `### ${uiText("Tool input", "Đầu vào công cụ")}\n${formatPayloadForSidebar(
        card.inputText,
        inputIsJson ? "json" : "text",
      )}`,
    );
  }
  ...
  if (card.outputText?.trim()) {
    ...
    sections.push(
      `### ${uiText("Tool output", "Đầu ra công cụ")}\n*${uiText(
        ...
        "Không có đầu ra - công cụ đã hoàn tất thành công.",
      )}*`,
    );
  }
  ...
  return html`
    <div class="chat-tool-card__raw">
      ...
      <div class="chat-tool-card__raw-body" hidden>
        ${renderToolDataBlock({
          label: uiText("Tool output", "Đầu ra công cụ"),
          text,
          expanded: true,
        })}
      </div>
...
export function renderExpandedToolCardContent(
  ...
  return html`
    <div class="chat-tool-card chat-tool-card--expanded">
      ...
      ${hasInput
        ? renderToolDataBlock({
            label: uiText("Tool input", "Đầu vào công cụ"),
            text: card.inputText!,
            expanded: true,
          })
        : nothing}
