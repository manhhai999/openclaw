...
function execSecurityLabel(value: ExecSecurity): string {
  return t(`nodes.execApprovals.options.security.${value}`);
}

function execAskLabel(value: ExecAsk): string {
  return t(`nodes.execApprovals.options.ask.${value}`);
}

function execBooleanLabel(value: boolean): string {
  return value ? t("nodes.execApprovals.on") : t("nodes.execApprovals.off");
}
    ...
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">${t("nodes.execApprovals.target.subtitle")}</div>
        </div>
        <div class="list-meta">
          <label class="field">
          ...
          ${state.target === "node"
            ? html`
                <label class="field">
                  <span>${t("nodes.execApprovals.target.node")}</span>
                  <select
...
function renderExecApprovalsPolicy(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes.execApprovals.security.title")}</div>
          <div class="list-sub">
            ${isDefaults
              ? t("nodes.execApprovals.security.defaultSubtitle")
              : t("nodes.execApprovals.defaultValue", {
                  value: execSecurityLabel(defaults.security),
                })}
          </div>
        </div>
        <div class="list-meta">
              ...
              ${SECURITY_OPTIONS.map(
                (option) =>
                  html`<option value=${option.value} ?selected=${securityValue === option.value}>
                    ${execSecurityLabel(option.value)}
                  </option>`,
              )}
            </select>
          </label>
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes.execApprovals.ask.title")}</div>
          <div class="list-sub">
            ${isDefaults
              ? t("nodes.execApprovals.ask.defaultSubtitle")
              : t("nodes.execApprovals.defaultValue", {
                  value: execAskLabel(defaults.ask),
                })}
          </div>
        </div>
        <div class="list-meta">
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes.execApprovals.askFallback.title")}</div>
          <div class="list-sub">
            ${isDefaults
              ? t("nodes.execApprovals.askFallback.defaultSubtitle")
              : t("nodes.execApprovals.defaultValue", {
                  value: execSecurityLabel(defaults.askFallback),
                })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodes.execApprovals.askFallback.fallback")}</span>
            <select
              ?disabled=${state.disabled}
              @change=${(event: Event) => {
                const target = event.target as HTMLSelectElement;
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("nodes.execApprovals.autoAllowSkills.title")}</div>
          <div class="list-sub">
            ${isDefaults
              ? t("nodes.execApprovals.autoAllowSkills.defaultSubtitle")
              : autoIsDefault
                ? t("nodes.execApprovals.autoAllowSkills.usingDefault", {
                    value: execBooleanLabel(defaults.autoAllowSkills),
                  })
                : t("nodes.execApprovals.autoAllowSkills.overrideValue", {
                    value: execBooleanLabel(autoEffective),
                  })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            <span>${t("nodes.execApprovals.autoAllowSkills.enabled")}</span>
            <input
              type="checkbox"
              ?disabled=${state.disabled}
...
function renderExecApprovalsAllowlist(state: ExecApprovalsState) {
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        <div class="card-title">${t("nodes.execApprovals.allowlist.title")}</div>
        <div class="card-sub">${t("nodes.execApprovals.allowlist.subtitle")}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
      ...
      >
        ${t("nodes.execApprovals.allowlist.addPattern")}
      </button>
    </div>
    <div class="list" style="margin-top: 12px;">
      ${entries.length === 0
        ? html` <div class="muted">${t("nodes.execApprovals.allowlist.empty")}</div> `
...
function renderAllowlistEntry(
  ...
  const lastUsed = entry.lastUsedAt
    ...
    : t("nodes.execApprovals.allowlist.never");
    ...
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">
          ${entry.pattern?.trim() ? entry.pattern : t("nodes.execApprovals.allowlist.newPattern")}
        </div>
        <div class="list-sub">
          ${t("nodes.execApprovals.allowlist.lastUsed", { value: lastUsed })}
        </div>
      ...
      <div class="list-meta">
        <label class="field">
          <span>${t("nodes.execApprovals.allowlist.pattern")}</span>
          <input
            type="text"
            .value=${entry.pattern ?? ""}
            ?disabled=${state.disabled}
        ...
        >
          ${t("nodes.execApprovals.allowlist.remove")}
        </button>
      </div>
    </div>
  `;
