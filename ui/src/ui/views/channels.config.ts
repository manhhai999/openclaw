...
const EXTRA_CHANNEL_FIELD_LABELS: Record<(typeof EXTRA_CHANNEL_FIELDS)[number], string> = {
  groupPolicy: "channels.config.extraFields.groupPolicy",
  streamMode: "channels.config.extraFields.streamMode",
  dmPolicy: "channels.config.extraFields.dmPolicy",
};
...
function renderExtraChannelFields(value: Record<string, unknown>) {
  const entries = EXTRA_CHANNEL_FIELDS.flatMap((field) => {
    ...
    return [[field, value[field]]] as Array<[(typeof EXTRA_CHANNEL_FIELDS)[number], unknown]>;
  });
  if (entries.length === 0) {
    return null;
  }
  return html`
    <div class="status-list" style="margin-top: 12px;">
      ${entries.map(
        ([field, raw]) => html`
          <div>
            <span class="label">${t(EXTRA_CHANNEL_FIELD_LABELS[field])}</span>
