  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nodes.page.title")}</div>
          <div class="card-sub">${t("nodes.page.subtitle")}</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${props.nodes.length === 0
          ? html` <div class="muted">${t("nodes.page.empty")}</div> `
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ${t("nodes.devices.role")}: ${roleValue} · ${t("nodes.devices.scopes")}: ${scopesValue} ·
          ${t("nodes.devices.requested")} ${age}${repair}
        </div>
      </div>
      <div class="list-meta">
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = `${t("nodes.devices.roles")}: ${formatList(device.roles)}`;
  const scopes = `${t("nodes.devices.scopes")}: ${formatList(device.scopes)}`;
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  ...
  const scopes = `${t("nodes.devices.scopes")}: ${formatList(token.scopes)}`;
...
function renderNode(node: Record<string, unknown>) {
  ...
  const title =
    ...
    (typeof node.nodeId === "string" ? node.nodeId : t("nodes.page.unknown"));
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip"
            >${paired ? t("nodes.status.paired") : t("nodes.status.unpaired")}</span
          >
