  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nodes.overview.title")}</div>
          <div class="card-sub">${t("nodes.overview.subtitle")}</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${props.nodes.length === 0
          ? html` <div class="muted">${t("nodes.overview.empty")}</div> `
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const summaryParts = [
    `${t("nodes.devices.roleLabel")} ${roleValue}`,
    `${t("nodes.devices.scopesLabel")} ${scopesValue}`,
    t("nodes.devices.requested", { time: age }),
  ];
  if (req.isRepair) {
    summaryParts.push(t("nodes.devices.repair"));
  }
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">${summaryParts.join(" · ")}</div>
      </div>
      <div class="list-meta">
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = `${t("nodes.devices.rolesLabel")} ${formatList(device.roles)}`;
  const scopes = `${t("nodes.devices.scopesLabel")} ${formatList(device.scopes)}`;
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        ${tokens.length === 0
          ...
          : html`
              <div class="muted" style="margin-top: 10px;">${t("nodes.devices.tokensTitle")}</div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? t("nodes.devices.revoked") : t("nodes.devices.active");
  const scopes = `${t("nodes.devices.scopesLabel")} ${formatList(token.scopes)}`;
            ...
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                ...
                <div class="list-meta">
                  ...
                  ${!supportsBinding
                    ? html` <div class="muted">${t("nodes.binding.noExecNodes")}</div> `
                    : nothing}
                </div>
              </div>
        ...
        <div class="list-sub">
          ...
          ${bindingValue === "__default__"
            ? t("nodes.binding.usesDefault", {
                value: state.defaultBinding ?? t("nodes.binding.any"),
              })
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">
            ${paired ? t("nodes.overview.paired") : t("nodes.overview.unpaired")}
          </span>
