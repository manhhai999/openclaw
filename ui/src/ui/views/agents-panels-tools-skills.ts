...
function renderToolBadges(section: AgentToolSection, tool: AgentToolEntry) {
  ...
  if (source === "plugin" && pluginId) {
    ...
    badges.push(t("agentsPage.tools.badges.core"));
  }
  if (tool.optional) {
    badges.push(t("agentsPage.tools.badges.optional"));
  }
  if (badges.length === 0) {
    return nothing;
  }
...
export function renderAgentTools(params: {
  ...
  const profileSource = agentTools.profile
    ? t("agentsPage.tools.sourceAgentOverride")
    : globalTools.profile
      ? t("agentsPage.tools.sourceGlobalDefault")
      : t("agentsPage.tools.sourceDefault");
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; flex-wrap: wrap;">
        <div style="min-width: 0;">
          <div class="card-title">${t("agentsPage.tabs.tools")}</div>
          <div class="card-sub">
      ...
      ${hasAgentAllow
        ? html`
            <div class="callout info" style="margin-top: 12px">
              ${t("agentsPage.tools.explicitAllowlist")}
            </div>
          `
        : nothing}
      ${hasGlobalAllow
        ? html`
            <div class="callout info" style="margin-top: 12px">
              ${t("agentsPage.tools.globalAllowlist")}
            </div>
          `
        : nothing}
      ${params.toolsCatalogLoading && !params.toolsCatalogResult && !params.toolsCatalogError
      ...
      <div class="agent-tools-meta" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${t("agentsPage.tools.profile")}</div>
          <div class="mono">${profile}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t("agentsPage.tools.source")}</div>
          <div>${profileSource}</div>
        </div>
        ${params.configDirty
          ? html`
              <div class="agent-kv">
                <div class="label">${t("agentsPage.tools.status")}</div>
                <div class="mono">${t("agentsPage.tools.statusUnsaved")}</div>
              </div>
            `
          : nothing}
      </div>
      ...
      <div style="margin-top: 18px;">
        <div class="label">${t("agentsPage.tools.availableNow")}</div>
        <div class="card-sub">
          ${t("agentsPage.tools.availableNowSubtitle")}
          <span class="mono">${params.runtimeSessionKey || t("agentsPage.tools.noSession")}</span>
        </div>
        ${!params.runtimeSessionMatchesSelectedAgent
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agentsPage.tools.switchChatHint")}
              </div>
            `
          : params.toolsEffectiveLoading &&
            ...
            ? html`
                <div class="callout info" style="margin-top: 12px">
                  ${t("agentsPage.tools.loadingAvailable")}
                </div>
              `
            : params.toolsEffectiveError
              ? html`
                  <div class="callout info" style="margin-top: 12px">
                    ${t("agentsPage.tools.availableError")}
                  </div>
                `
              : (params.toolsEffectiveResult?.groups?.length ?? 0) === 0
                ? html`
                    <div class="callout info" style="margin-top: 12px">
                      ${t("agentsPage.tools.noneAvailable")}
                    </div>
                  `
                : html`
                        ...
                        (group) => html`
                          <div class="agent-tools-section">
                            <div class="agent-tools-header">
                              ${localizeToolSectionLabel(group.id, group.label)}
                            </div>
      ...
      <div class="agent-tools-presets" style="margin-top: 16px;">
        <div class="label">${t("agentsPage.tools.quickPresets")}</div>
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; flex-wrap: wrap;">
        ...
        <div class="row" style="gap: 8px; flex-wrap: wrap;">
          ...
          >
            ...
            >
              ${t("agentsPage.skills.disableAll")}
            </button>
            <button
              class="btn btn--sm"
              ?disabled=${!editable || !usingAllowlist}
              @click=${() => params.onClear(params.agentId)}
              title="${t("agentsPage.skills.resetTitle")}"
            >
              ${t("common.reset")}
            </button>
          </div>
      ...
      ${!params.configForm
        ? html`
            <div class="callout info" style="margin-top: 12px">
              ${t("agentsPage.skills.loadConfigHint")}
            </div>
          `
        : nothing}
      ${usingAllowlist
        ? html`
            <div class="callout info" style="margin-top: 12px">
              ${t("agentsPage.skills.customAllowlist")}
            </div>
          `
        : html`
            <div class="callout info" style="margin-top: 12px">
              ${t("agentsPage.skills.allEnabledHint")}
            </div>
          `}
      ${!reportReady && !params.loading
        ? html`
            <div class="callout info" style="margin-top: 12px">
              ${t("agentsPage.skills.loadHint")}
            </div>
          `
        : nothing}
      ${params.error
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field" style="flex: 1;">
          ...
          <input
            ...
            placeholder="${t("agentsPage.skills.searchPlaceholder")}"
            autocomplete="off"
            name="agent-skills-filter"
          />
        </label>
        <div class="muted">
          ${t("agentsPage.skills.shownCount", { count: String(filtered.length) })}
        </div>
      </div>

      ${filtered.length === 0
        ? html`
            <div class="muted" style="margin-top: 16px">${t("agentsPage.skills.noSkills")}</div>
          `
        : html`
        ...
        ${missing.length > 0
          ? html`
              <div class="muted" style="margin-top: 6px;">
                ${t("agentsPage.skills.missing", { value: missing.join(", ") })}
              </div>
            `
          : nothing}
        ${reasons.length > 0
          ? html`
              <div class="muted" style="margin-top: 6px;">
                ${t("agentsPage.skills.reason", { value: reasons.join(", ") })}
              </div>
            `
          : nothing}
      </div>
