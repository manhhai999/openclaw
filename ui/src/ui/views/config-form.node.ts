  ...
  return html`
    <button
      ...
      title=${state.canReveal
        ? state.isRevealed
          ? t("settings.form.hideValue")
          : t("settings.form.revealValue")
        : t("settings.form.streamModeReveal")}
...
export function renderNode(params: {
  ...
  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">${t("settings.form.unsupportedSchemaNode")}</div>
    </div>`;
  }
  if (
    criteria &&
  ...
  return html`
    <div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">
        ${t("settings.form.unsupportedType", { type: type ?? "" })}
      </div>
    </div>
  `;
}
...
function renderTextInput(params: {
  ...
  const placeholder = effectiveRedacted
    ? isStructuredSecretRef
      ? rawAvailable
        ? t("settings.form.secretRefUseRaw")
        : t("settings.form.secretRefEditFile")
      : REDACTED_PLACEHOLDER
    : (hint?.placeholder ??
      (schema.default !== undefined
        ? t("settings.form.defaultValue", { value: formatUnknownText(schema.default) })
        : ""));
        ...
        ${schema.default !== undefined
          ? html`
              <button
                ...
                title="${t("settings.form.resetToDefault")}"
...
function renderSelect(params: {
  ...
  return html`
    <div class="cfg-field">
      ...
      >
        <option value=${unset}>${t("settings.form.selectPlaceholder")}</option>
...
function renderArray(params: {
  ...
  if (!itemsSchema) {
    return html`
      <div class="cfg-field cfg-field--error">
        <div class="cfg-field__label">${label}</div>
        <div class="cfg-field__error">${t("settings.form.unsupportedArraySchema")}</div>
      </div>
    `;
  }
    ...
    <div class="cfg-array">
      <div class="cfg-array__header">
        ...
        <span class="cfg-array__count">
          ${t("settings.form.itemCount", { count: String(arr.length) })}
        </span>
        <button
          type="button"
      ...
      ${arr.length === 0
        ? html` <div class="cfg-array__empty">${t("settings.form.emptyArray")}</div> `
        : html`
                  ...
                  <div class="cfg-array__item">
                    <div class="cfg-array__item-header">
                      ...
                      <button
                        ...
                        title="${t("settings.form.removeItem")}"
                        ?disabled=${disabled}
                        @click=${() => {
                          const next = [...arr];
                          next.splice(idx, 1);
  ...
  return html`
    <div class="cfg-map">
      <div class="cfg-map__header">
        <span class="cfg-map__label">${t("settings.form.customEntries")}</span>
        <button
          type="button"
          class="cfg-map__add"
          ?disabled=${disabled}
        ...
        >
          ...
          ${t("settings.form.addEntry")}
        </button>
      </div>

      ${visibleEntries.length === 0
        ? html` <div class="cfg-map__empty">${t("settings.form.noCustomEntries")}</div> `
        : html`
                    ...
                    <div class="cfg-map__item-header">
                      <div class="cfg-map__item-key">
                        <input
                          ...
                          placeholder="${t("settings.form.key")}"
                          .value=${key}
                          ?disabled=${disabled}
                          @change=${(e: Event) => {
                      ...
                      <button
                        ...
                        title="${t("settings.form.removeEntry")}"
                        ?disabled=${disabled}
                        @click=${() => {
                          const next = { ...value };
                          delete next[key];
